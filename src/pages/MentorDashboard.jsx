        ...
        <main className="main">
          <div className="grid">
            ...
            {activePage === "dashboardPage" && (
              <section className="card" id="dashboardPage" style={{ gridColumn: "1 / -1" }}>
                <h3>Overview</h3>
                <p className="muted">This view surfaces at-risk students early using simple rule-based logic. Upload CSVs from Attendance, Exams and Fees, or use the sample dataset.</p>

                <div style={{ height: 12 }}></div>

                {/* small controls */}
                <div className="controls">
                  <input className="input" id="searchInput" placeholder="Search by name or id" value={search} onChange={(e) => setSearch(e.target.value)} style={{ maxWidth: 240 }} />
                  <select id="filterRisk" value={riskFilter} onChange={(e) => setRiskFilter(e.target.value)}>
                    <option value="all">Show: All</option>
                    <option value="high">High risk</option>
                    <option value="medium">Medium risk</option>
                    <option value="low">Low risk</option>
                  </select>
                  <button className="btn" onClick={() => alert("Risk recalculated automatically.")}>Recompute Risk</button>
                  <button className="btn ghost" onClick={() => { setMentor(cloneMentor(DEFAULT_MENTOR)); alert("Sample data loaded."); }}>Load Sample Data</button>
                  <div style={{ marginLeft: "auto" }} className="muted">Auto-persist: <b>localStorage</b></div>
                </div>

                <div style={{ height: 12 }}></div>

                <h4 style={{ margin: "0 0 8px 0" }}>Your Mentees</h4>
                <div className="student-list" id="menteeList">
                  {filteredRisk.map((r) => {
                    const s = mentor.mentees.find((x) => x.id === r.id) || {};
                    return (
                      <div className="student-card" key={r.id}>
                        <div className="student-meta">
                          <div className="avatar">{(s.name || s.id || "").split(" ").map((p) => p[0]).slice(0, 2).join("")}</div>
                          <div>
                            <b>{s.name || r.id}</b>
                        ...
                        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                          <div className="muted">Att: <b>{r.attendance}%</b></div>
                          <div className="muted">Avg: <b>{r.avgScore !== null ? r.avgScore.toFixed(1) : "-"}</b></div>
                          <div className="muted">Fees: <b>{rupee(r.feesPending || 0)}</b></div>
                          <div><span className={`risk-b ${r.risk === "High" ? "risk-high" : r.risk === "Medium" ? "risk-med" : "risk-low"}`}>{r.risk}</span></div>
                          <div style={{ display: "flex", flexDirection: "column", gap: 6, marginLeft: 10 }}>
                            <button className="btn ghost" onClick={() => viewStudent(r.id)}>View</button>
                            <button className="btn primary" onClick={() => messageStudent(r.id)}>Message</button>
                          </div>
                        </div>
                      </div>
                    );
                ...
                <h4 style={{ margin: "0 0 8px 0" }}>Risk Table</h4>
                <div style={{ height: 8 }}></div>
                <table id="riskTable">
                  <thead>
                    <tr><th>ID</th><th>Name</th><th>Attendance</th><th>Avg Score</th><th>Attempts</th><th>Fees Pending</th><th>Issues</th><th>Risk</th><th>Action</th></tr>
                  </thead>
                  <tbody>
                    {riskData.map((r) => {
                      const issues = r.issues.length ? r.issues.join(", ") : "-";
                      return (
                        <tr key={r.id} className={r.risk === "High" ? "row-high" : r.risk === "Medium" ? "row-med" : "row-low"}>
                          <td>{r.id}</td>
                          <td>{r.name}</td>
                          <td>{r.attendance}%</td>
                          <td>{r.avgScore !== null ? r.avgScore.toFixed(1) : "-"}</td>
                          <td>{r.attempts || 0}</td>
                          <td>{rupee(r.feesPending || 0)}</td>
                          <td>{issues}</td>
                          <td><span className={`risk-b ${r.risk === "High" ? "risk-high" : r.risk === "Medium" ? "risk-med" : "risk-low"}`}>{r.risk}</span></td>
                          <td><button className="btn ghost" onClick={() => viewStudent(r.id)}>View</button></td>
                        </tr>
                      );
                    })}
                  </tbody>
                ...
                <div style={{ height: 12 }}></div>
                <h4 style={{ margin: "0 0 8px 0" }}>Notifications Preview</h4>
                <textarea
                  id="notifPreview"
                  readOnly
                  style={{ width: "100%", height: 120, background: "var(--soft)", color: "var(--text-dark)", padding: 10, borderRadius: 8, border: "1px solid rgba(255,255,255,0.04)" }}
                  value={riskData
                    .filter((r) => r.risk !== "Low")
                    .map((r) => `Student ${r.name || r.id} (${r.id}) — ${r.risk} risk — Issues: ${r.issues.join(", ")}`)
                    .join("\n\n")}
                />
              </section>
            )}
